...
export const ProductDetail = () => {
  ...
  if (error) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4">
      ...
      <Link to="/products" className="px-6 py-2 bg-gold text-white rounded-full font-bold">Back to Shop</Link>
    </div>
  );
  ...
  const waMessage = `Hi Annaya Boutique! I'm interested in ${product.name} - Size: ${selectedSize}, Color: ${selectedColor?.name}, Price: ${formatCurrency(product.price)}. Please assist me.`;
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="flex items-center gap-1 px-3 py-1 bg-gold/10 text-gold rounded-full text-sm font-bold">
                <Star className="w-4 h-4 fill-gold" /> {Number(product.rating || 0).toFixed(1)}
              </div>
            ...
            <div className="bg-sky/10 p-6 rounded-3xl mb-8">
              <div className="flex items-baseline gap-4">
                <span className="text-4xl font-bold text-royal">{formatCurrency(product.price)}</span>
            ...
            <div className="space-y-8">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-900 uppercase tracking-widest mb-4">
                  Color: <span className="text-slate-500 font-medium normal-case">{selectedColor?.name}</span>
                </label>
                  ...
                  {product.colors.map((color: any) => (
                    <button
                      ...
                      className={cn(
                        ...
                        selectedColor?.name === color.name ? "border-royal scale-110" : "border-transparent"
                      )}
                      style={{ backgroundColor: color.hex }}
                      title={color.name}
                    >
                      {selectedColor?.name === color.name && (
                        <div className="absolute inset-[-4px] border border-royal rounded-full"></div>
                      )}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isInWishlist(product._id) ? "border-rose-100 bg-rose-50 text-rose-500" : "border-slate-100 text-slate-400 hover:border-rose-100"
                  )}
                >
                  <Heart className={cn("w-6 h-6", isInWishlist(product._id) && "fill-current")} />
                </button>
              </div>
            </div>
                ...
                {["description", "details", "shipping"].map((tab) => (
                  <button
                    ...
                    className={cn(
                      ...
                      activeTab === tab ? "border-royal text-royal" : "border-transparent text-slate-400"
                    )}
                  >
                    {tab}
                  </button>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p>On all orders above ₹999. Standard delivery takes 4-7 business days.</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-3">
                      <RefreshCw className="w-5 h-5 text-gold shrink-0" />
                      <div>
                        <p className="font-bold text-slate-900">Easy Returns & Exchange</p>
                        <p>7-day hassle-free return policy for unworn items with original tags.</p>
                      </div>
                    </div>
                  </div>
                )}
